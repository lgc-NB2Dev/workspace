...
- IMPORTANT: Never run `uv sync`, `uv run`, or other dependency commands from inside `private/test-nb2` if they may create or update a nested virtual environment. To run the test project, activate or reuse the workspace root virtual environment first, then run `nb run` in `private/test-nb2`.
...
- This workspace uses `markdown-it-py` for Markdown rendering. Markdown render rules are called with 4 arguments `(tokens, idx, options, env)`, not 5 — there is no `slf` parameter in v4.x. When overriding render rules, capture the original rule as a closure variable before replacing it; do not rely on `self` binding.
